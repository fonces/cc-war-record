...
const calculateMapJobSummaries = (matchRecords: MatchRecord[]): MapJobSummaries[] => {
  ...
  return allMaps.map((map) => {
    ...
    jobMap.forEach((summary) => {
      summary.winRate = summary.totalMatches > 0 
      ...
      {mapJobSummaries.map((mapData) => (
        <StyledMapSection key={mapData.map}>
          <StyledMapTitle onClick={() => toggleMap(mapData.map)}>
            <StyledMapTitleLeft>
              <Icon 
                name={openMaps.has(mapData.map) ? 'minus' : 'add'} 
                size={16} 
              />
            ...
            <StyledMapSummary>
              ...
              <span>{mapData.totalWins}勝{mapData.totalLosses}敗</span>
              <StyledMapWinRate winRate={mapData.mapWinRate}>
                勝率{mapData.mapWinRate}%
              </StyledMapWinRate>
            </StyledMapSummary>
          </StyledMapTitle>
            ...
            <StyledTable>
              ...
              <StyledTableBody>
                {usedJobs.length === 0 ? (
                  ...
                  usedJobs.map((job) => {
                    const summary = mapData.jobSummaries.find(s => s.job === job) || { job, totalMatches: 0, wins: 0, losses: 0, winRate: 0 }
                    return (
      ...
      <StyledMapSection>
        <StyledMapTitle style={{ cursor: 'default' }}>
          ...
          <StyledMapSummary>
            ...
            <span>{totalWins}勝{totalLosses}敗</span>
            <StyledMapWinRate winRate={totalWinRate}>
              勝率{totalWinRate}%
            </StyledMapWinRate>
          </StyledMapSummary>
        </StyledMapTitle>
          ...
          <StyledTable>
            ...
            <StyledTableBody>
              {usedJobs.length === 0 ? (
                ...
                usedJobs.map((job) => {
                  const summary = totalSummaries.find(s => s.job === job) || { job, totalMatches: 0, wins: 0, losses: 0, winRate: 0 }
                  return (
                    <StyledTableRow key={job}>
                      <StyledJobCell>
                        <div>
