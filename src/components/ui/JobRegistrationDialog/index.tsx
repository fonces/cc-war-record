...
const StyledJobCard = styled.button<{ isSelected: boolean; isDisabled?: boolean }>`
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: ${({ theme }) => theme.spacing[2]};
  padding: ${({ theme }) => theme.spacing[3]};
  border: 2px solid ${({ isSelected, isDisabled, theme }) => 
    isDisabled ? theme.colors.gray[200] :
    isSelected ? theme.colors.primary[500] : theme.colors.gray[200]};
  border-radius: ${({ theme }) => theme.borderRadius.md};
  background-color: ${({ isSelected, isDisabled, theme }) => 
    isDisabled ? theme.colors.gray[100] :
  ...
  &:hover {
    border-color: ${({ isDisabled, theme }) => 
      isDisabled ? theme.colors.gray[200] : theme.colors.primary[300]};
    background-color: ${({ isDisabled, theme }) => 
      isDisabled ? theme.colors.gray[100] : theme.colors.primary[50]};
  }
`
 ...
 * ジョブ登録ダイアログコンポーネント
 */
export const JobRegistrationDialog = ({
  isOpen,
  onClose,
  onRegister,
  characterUuid,
  ...
  const registeredJobs = history?.characterStats
    .find(cs => cs.character.uuid === characterUuid)?.usedJobs || []
