...
| `createHistory(input)` | 新しいシーズン履歴を作成 | `CreateHistoryInput` | `History` |
| `updateHistory(uuid, input)` | 履歴を更新 | `uuid: string`, `UpdateHistoryInput` | `boolean` |
| `deleteHistory(uuid)` | 履歴を削除 | `uuid: string` | `boolean` |
...
const MyComponent = () => {
  ...
  const getCharacterStatsForSeason = useCharacterStore(
    state => state.getCharacterStatsForSeason
  )
...
- `cc-war-record-match-records`: 戦績記録データ
...
export const useSeasonStats = (seasonUuid: string) => {
  const stats = useCharacterStore(
    state => state.getCharacterStatsForSeason(seasonUuid)
  )
